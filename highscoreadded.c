...
#include<windows.h>
#include<conio.h>
#include "highscore.c"  // Include the highscore functions
...
#define HYEL "\e[0;93m" //High Intensity Yellow for caution message
#define BHRED "\e[1;91m" //bold high intensity red for game over
#define HGRN "\e[0;92m" // Green Congrats
#define RESET "\e[0m" 
...
// Function to check for zombies in front and handle elimination
 int checkForZombiesAndEliminate(char direction, int* playerX, int* playerY, char maze[MAZE_SIZE][MAZE_SIZE], char playerChar, int* playerEliminations) {
    ...
    if (direction == 'W' || direction == 'w') {
     ...
     if (zombieInFront) {
    printf(HYEL"Warning: Zombie in front! Press 'O' twice to eliminate it.\n"RESET);
...
int timeLeft = timeLimit - (currentTime - startTime);
    
// Score calculation and high score handling
    for (int i = 0; i < (playerMode == '2' ? 2 : 1); i++) {
        int playerScore = calculateScore(eliminations[i], timeLeft, mode);
        char playerName[10];
        snprintf(playerName, sizeof(playerName), "Player %c", playerChar[i]);

        printf("%s's score: %d\n", playerName, playerScore);
        saveHighscore(playerName, playerScore, mode);
    }

    // Display highscores after game ends
displayHighscores(singlePlayerFile, player1File, player2File);
